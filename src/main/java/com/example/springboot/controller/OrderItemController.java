...
public class OrderItemController {
    ...
    public Result save(@RequestBody OrderItem orderItem) {
        ...
        OrderItem orderItem1 = orderItemService.getById(orderItem.getId());
        if (StrUtil.isBlank(orderItem1.getComment()) && StrUtil.isNotBlank(orderItem.getComment())) {
    ...
    public Result comment(@PathVariable Integer goodsId) {
        ...
        for (OrderItem orderItem : list) {
            ...
            if (StrUtil.isNotBlank(orderItem.getComment())) {
                comments.add(Dict.create().set("avatar", user.getAvatarUrl()).set("user", user.getNickname()).set("scores", orderItem.getScores()).set("comment", orderItem.getComment()));
            }
        }
